...
timeaa=request.getParameter("time");
iplist=iplook.findIpDay(timeaa);
}
...
<input type="submit" value="时间查询"><input type="reset" value="重置">
 	...
 	<table class="pagelist" width="600">
						...
						%>
								<a href="visit1.jsp?pagenumb=<%=pagenm%>&time=<%=timeaa%>"><%=pagenm%> </a>	
						<%
							}
						%>
								<a href="visit1.jsp?pagenumb=<%=pagenum + 1%>&time=<%=timeaa%>"> 下一页 </a>
								<a href="visit1.jsp?pagenumb=<%=totalpage%>&time=<%=timeaa%>"> 尾页 </a>
						<% 
							} else if (pagenum == totalpage&& totalpage>1) {
						%>	
							<a href="visit1.jsp?pagenumb=1&time=<%=timeaa%>"> 首页 </a>
							<a href="visit1.jsp?pagenumb=<%=pagenum - 1%>&time=<%=timeaa%>"> 上一页 </a>
						<%
